...
if not os.getenv("OPENAI_API_KEY"):
    logging.error("DSA Log - Erro Crítico: Variável de ambiente OPENAI_API_KEY não encontrada!")
    logging.error("DSA Log - Certifique-se de que sua chave está no arquivo .env ou definida no sistema.")
    exit(1)  
...
def dsa_analise_sql_agente_ia(question: str):

    # Registra o início da análise com o agente
    logging.info(f"DSA Log - Iniciando Análise com SQL AI Agent Para a Pergunta: '{question}'")

    # Verifica se o arquivo do banco de dados existe
    if not os.path.exists(DUCKDB_FILE):
        logging.error(f"DSA Log - Erro: Arquivo de banco de dados '{DUCKDB_FILE}' não encontrado.")
        logging.error("DSA Log - Execute o script de scraping primeiro para criar e popular o banco de dados.")
        return  
    ...
    try:
        ...
        logging.info(f"DSA Log - Conectando ao DB via LangChain com URI: {db_uri}")
        ...
        logging.info(f"DSA Log - LLM inicializado: {llm.model_name}")
        ...
        logging.info("DSA Log - Agente SQL criado com sucesso.")

        # Registra o início da execução do agente com a pergunta fornecida
        logging.info(f"DSA Log - Invocando agente com a pergunta: '{question}'")

        # Invoca o agente com a pergunta e obtém a resposta
        response = agent_executor.invoke({"input": question})

        # Registra o término da execução do agente
        logging.info("DSA Log - Agente concluiu a execução.")

        # Registra a resposta final do agente
        logging.info("DSA Log - Resposta Final do Agente:")
        ...
        with open("dsa_perguntas_respostas.txt", "a", encoding = "utf-8") as arquivo:
            arquivo.write(f"{'='*40}\n")
        ...
        logging.error(f"DSA Log - Erro de importação. Verifique as instalações: {ie}")
        logging.error("DSA Log - Certifique-se de ter instalado as dependências.")

    except Exception as e:
        # Trata outros erros inesperados
        logging.error(f"DSA Log - Ocorreu um erro inesperado durante a análise com o agente: {e}", exc_info = True)
...
if __name__ == "__main__":
    ...
    dsa_analise_sql_agente_ia("Qual o preço médio dos livros?")
    dsa_analise_sql_agente_ia("Liste os 3 livros mais caros, incluindo seus preços.")
    dsa_analise_sql_agente_ia("Qual o livro mais barato?")
    dsa_analise_sql_agente_ia("Quantos livros custam mais de 50 libras?")
    dsa_analise_sql_agente_ia("Mostre o título e o preço do livro chamado 'Set Me Free'.")
